...
/* Inicializar as variaveis de estimulo de cada celula */
void setStimulus (Cell *c, bool stim)
{
    if (stim)
    {
        c->beats = 10;
    ...
    // As celulas que nao sao estimuladas pelo pacing possuem um BCL muito grande para tempo da simulacao
    else
    {
        c->beats = 10;
        c->BCL = TMAX + 1000;
...
/* Inicializar as variaveis de tempo da EDO */
...
/* Inicializar as condicoes iniciais de uma celula */
/* Valores default do LiRudy (2011) */
void setInitialConditions (Cell *c)
{
    c->v		= -84.058830;
    c->m		= 0.000821;
    c->h		= 0.995741;
...
void comp_istim (Cell *c, double t)
{
	c->stimtime += DT;
	if (t >= c->tstim)
	{
