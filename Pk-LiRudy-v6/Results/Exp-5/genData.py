...
for mesh in arr:
    ...
    for t in teta:
        ...
        # Plot the data
        pyplot.plot(data[:,0],data[:,1],label=str(perc[j]),linewidth=2,marker=markers[j],markersize=10)
        j = j + 1
    my_xticks = [0,1,2,3]
    ...
    #pyplot.title(u'Activation Time x Level - (LiRudy)',fontsize=15)
    pyplot.legend(loc=0,fontsize=14)
    pyplot.grid()
    pyplot.savefig("lvl-at-"+str(mesh)+"-LiRudy-revision.pdf")
    #pyplot.show()
