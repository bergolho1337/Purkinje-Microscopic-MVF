...
void Solver::solveODE (double t)
{
    ...
    while (ptr != NULL)
    {
        ...
        comp_revs(vol[id].cell);
        comp_ina (vol[id].cell);
        comp_inal (vol[id].cell);
        comp_inab (vol[id].cell);
        comp_ical (vol[id].cell);
        comp_icat (vol[id].cell);
        comp_icab (vol[id].cell);
        comp_ito1 (vol[id].cell);
        comp_ikr (vol[id].cell);
        comp_iks (vol[id].cell);
        comp_ik1 (vol[id].cell);
        comp_inaca (vol[id].cell);
        comp_inak (vol[id].cell);
        comp_ipca (vol[id].cell);
        comp_if (vol[id].cell);
        comp_istim (vol[id].cell,t);
        comp_itot (vol[id].cell);

        comp_ip3 (vol[id].cell);
        comp_qrel1 (vol[id].cell);
        comp_qrel2 (vol[id].cell);
        comp_qup1 (vol[id].cell);
        comp_qup2 (vol[id].cell);
        comp_qtr1 (vol[id].cell);
        comp_qtr2 (vol[id].cell);

        comp_conc (vol[id].cell);

        double dvdt	           = -vol[id].cell->itot;
        vol[id].cell->v	       += dvdt*dt;
        ptr = ptr->next;
